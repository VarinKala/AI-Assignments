...
from code_2022561 import create_kb  # Ensure the data is loaded for testing
from code_2022561 import get_busiest_routes  # New functions for testing
from code_2022561 import (
    backward_chaining,
...
test_inputs = {
    ...
    "busiest_routes": [[(123, 456), (789, 234), (567, 235), (3456, 897), (345, 345)]],
    "most_frequent_stops": [
    ...
    for (start_stop, end_stop), expected_output in test_inputs["direct_route"]:
        ...
        print(
            ...
            (
                ...
                else f"Fail (Expected: {expected_output}, Got: {actual_output})"
            ),
        )
...
def test_query_direct_routes():
    for (start_stop, end_stop), expected_output in test_inputs["direct_route"]:
        ...
        print(
            f"Test query_direct_routes ({start_stop}, {end_stop}): ",
            (
                "Pass"
                if check_output(expected_output, actual_output)
                else f"Fail (Expected: {expected_output}, Got: {actual_output})"
...
def test_forward_chaining():
    for (start_stop, end_stop, via_stop, max_transfers), expected_output in test_inputs[
        "forward_chaining"
    ]:
...
def test_backward_chaining():
    for (end_stop, start_stop, via_stop, max_transfers), expected_output in test_inputs[
        "backward_chaining"
    ]:
...
def test_pddl_planning():
    for (start_stop, end_stop, via_stop, max_transfers), expected_output in test_inputs[
        ...
        print(
            f"Test pddl_planning ({start_stop}, {end_stop}, {via_stop}, {max_transfers}): ",
            (
                "Pass"
                if check_output(expected_output, actual_output)
                else f"Fail (Expected: {expected_output}, Got: {actual_output})"
...
def test_bfs_route_planner():
    for (
        ...
        initial_fare,
        max_transfers,
    ), expected_output in test_inputs["bfs_route"]:
        pruned_df = prune_data(merged_fare_df, initial_fare)
        route_summary = compute_route_summary(pruned_df)
        actual_output = bfs_route_planner_optimized(
            start_stop, end_stop, initial_fare, route_summary, max_transfers
        )
        print(
            f"Test bfs_route_planner_optimized ({start_stop}, {end_stop}, {initial_fare}, {max_transfers}): ",
            (
                "Pass"
                if check_output(expected_output, actual_output)
                else f"Fail (Expected: {expected_output}, Got: {actual_output})"
...
def test_get_most_frequent_stops():
    ...
    print(
        f"Test get_most_frequent_stops: ",
        (
            "Pass"
            if check_output(expected_output, actual_output)
            else f"Fail (Expected: {expected_output}, Got: {actual_output})"
...
def test_get_top_5_busiest_stops():
    ...
    print(
        f"Test get_top_5_busiest_stops: ",
        (
            "Pass"
            if check_output(expected_output, actual_output)
            else f"Fail (Expected: {expected_output}, Got: {actual_output})"
...
def test_get_stops_with_one_direct_route():
    ...
    print(
        f"Test get_stops_with_one_direct_route: ",
        (
            "Pass"
            if check_output(expected_output, actual_output)
            else f"Fail (Expected: {expected_output}, Got: {actual_output})"
